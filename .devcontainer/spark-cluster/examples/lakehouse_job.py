...
spark-submit.sh fetches all credentials before submitting (Keycloak client-
credentials grant -> Unity Catalog token + temporary S3 credentials from
SeaweedFS STS), so this job contains no auth code. Without those credentials
the same operations fail: Unity Catalog returns 401 and SeaweedFS returns 403.
